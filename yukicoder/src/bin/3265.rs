...
fn main() {
    input! {
        n: usize,
        m: usize,
        alr: [(i64,Usize1,Usize1); n],
        q: usize,
        xyuv: [(Usize1,Usize1,Usize1,Usize1); q],
    }

    let mut house_vec = vec![];
    let mut rating_segtree = Segtree::<Additive<i64>>::new(m);
    let mut hometown_lazy_segtree = LazySegtree::<F>::new(m);
    for (i, &(a, l, r)) in alr.iter().enumerate() {
        house_vec.push((i, a, l, r));
        rating_segtree.set(i, a);
        hometown_lazy_segtree.apply_range(l..=r, 1);
    }

    let mut ans = 0;
    for &(_, a, l, r) in &house_vec {
        let rating_sum = rating_segtree.prod(l..=r);
        ans += a * (r as i64 - l as i64 + 1) - rating_sum;
    }

    for &(x, y, u, v) in &xyuv {
        let (i, a, l, r) = house_vec[x];

        let old_rating_sum = rating_segtree.prod(l..=r);
        ans -= a * (r as i64 - l as i64 + 1) - old_rating_sum;

        let old_hometown_count =
            hometown_lazy_segtree.get(i) - if l <= i && i <= r { 1 } else { 0 };
        ans += a * old_hometown_count;

        rating_segtree.set(i, 0);
        hometown_lazy_segtree.apply_range(l..=r, -1);

        house_vec[x] = (y, a, u, v);
        rating_segtree.set(y, a);
        hometown_lazy_segtree.apply_range(u..=v, 1);

        let new_rating_sum = rating_segtree.prod(u..=v);
        ans += a * (v as i64 - u as i64 + 1) - new_rating_sum;

        let new_hometown_count =
            hometown_lazy_segtree.get(y) - if u <= y && y <= v { 1 } else { 0 };
        ans -= a * new_hometown_count;
